...
{
    public class NbaPlayerDataService
    {
        private readonly FantasyLeagueContext _context;
        private readonly HttpClient _httpClient;
        private readonly ILogger<NbaPlayerDataService> _logger;
        private const string API_KEY = "878201c9c8f8470b83954b7207d599b0";
        private const string API_URL = "https://api.sportsdata.io/v3/nba/scores/json/PlayersActiveBasic";
        private static readonly HashSet<string> ValidPositions = new() { "PG", "SG", "SF", "PF", "C", "G", "F" };

        public NbaPlayerDataService(FantasyLeagueContext context, HttpClient httpClient, ILogger<NbaPlayerDataService> logger)
        {
            _context = context;
        ...
        public async Task<SyncResult> SyncNbaPlayersAsync()
        {
            try
            {
                _logger.LogInformation("Starting NBA player data sync...");
                ...
                var players = JsonSerializer.Deserialize<List<ApiNbaPlayerData>>(jsonContent, new JsonSerializerOptions
                {
                ...
                var existingPlayers = await _context.ActiveNbaPlayers.ToDictionaryAsync(p => p.PlayerID);
                ...
                foreach (var apiPlayer in validPlayers)
                {
                    ...
                    {
                        // Add new player
                        var newPlayer = new ActiveNbaPlayer
                        {
                        ...
                        _context.ActiveNbaPlayers.Add(newPlayer);
                        addedCount++;
                    }
                }
                ...
                {
                    _context.ActiveNbaPlayers.RemoveRange(playersToRemove);
            ...
            {
                _logger.LogError(ex, "Error occurred during NBA player data sync");
                return new SyncResult { Success = false, Message = $"Error during sync: {ex.Message}" };
            }
        }

        public async Task<List<ActiveNbaPlayer>> GetActivePlayersAsync(string? position = null, string? team = null, int page = 1, int pageSize = 50)
        {
            var query = _context.ActiveNbaPlayers.AsQueryable();
        ...
        public async Task<ActiveNbaPlayer?> GetPlayerByIdAsync(int playerId)
        {
            return await _context.ActiveNbaPlayers
